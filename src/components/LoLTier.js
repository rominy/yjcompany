  ...
  useEffect(() => {
    const getLoLApi = async () => {
      try {
        ...
        const response = await Promise.all(members.map(member => axios.get(`/riotgames/lol/league/v4/entries/by-summoner/${member.summonerId}?api_key=${process.env.REACT_APP_RIOTGAMES_KEY}`)));
